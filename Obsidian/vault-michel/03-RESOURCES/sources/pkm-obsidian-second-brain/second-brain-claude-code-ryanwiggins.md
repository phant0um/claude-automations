...
- [[03-RESOURCES/concepts/pkm-obsidian/second-brain]] — implementação prática do conceito
- [[03-RESOURCES/concepts/llm-ml-foundations/vector-search]] — QMD; vsearch vs BM25
- [[03-RESOURCES/concepts/claude-code-tooling/claude-hooks]] — mecanismo de injeção automática de contexto
- [[03-RESOURCES/concepts/pkm-obsidian/knowledge-compounding]] — cada sessão enriquece a base

## Mecanismo em profundidade: por que hooks superam busca manual

O insight técnico central do sistema é que a injeção automática via hook elimina o custo cognitivo da busca. Sem o hook, o usuário precisa lembrar de buscar contexto relevante antes de cada tarefa — um atrito que, em prática, é frequentemente ignorado. Com o hook, o contexto relevante aparece *automaticamente* no bloco `<context>` sem intervenção do usuário.

A arquitetura dual-search (vsearch + BM25) resolve um problema de cobertura: buscas vetoriais capturam documentos semanticamente relacionados mesmo quando as palavras exatas não aparecem, mas falham em recuperar nomes próprios e siglas que aparecem literalmente. BM25 complementa exatamente esse ponto cego. Rodar ambos em paralelo e mesclar os resultados dá cobertura próxima de 100% sobre o corpus de 15k documentos.

## Por que 5 anos de documentos importam

O tamanho e a profundidade temporal da base de conhecimento é o diferencial real. Um sistema com 6 meses de histórico tem utilidade limitada. Com 5 anos, o sistema contém:
- Padrões de tomada de decisão que se repetem
- Contexto de projetos que foram descontinuados e depois retomados
- Feedback de performance que revela pontos cegos consistentes do usuário
- Relações entre pessoas e projetos que não estariam explícitas em documentos recentes

É o efeito composto da memória: quanto mais densa a base, mais rico o contexto injetado automaticamente em cada prompt.

## Limitações e considerações de privacidade

O sistema depende de acesso irrestrito a documentos de trabalho, o que cria riscos de privacidade significativos quando dados sensíveis (PII, dados de clientes, informações confidenciais de negócios) estão presentes. A solução descrita no artigo — anonimização com LLMs locais antes da indexação — adiciona uma etapa operacional que precisa ser mantida. Se a anonimização falhar silenciosamente, dados sensíveis entram no corpus sem aviso. Sistemas de produção precisam de validação periódica do processo de anonimização, não apenas da configuração inicial.
