...
**Achado:** 1 wikilink pré-existente quebrado (`04-SYSTEM/wiki/model-routing`, não existia) — descoberto durante split, não inventado link substituto.
**Fix (2026-06-19d):** página criada em [[04-SYSTEM/wiki/model-routing]] (resumo do model-router.md + link ADR-003); wikilink restaurado em `stopped-using-opus-everything-43-percent-cheaper.md`.

## Rotinas: hardening por aprendizados do vault (2026-06-19e)
**Motivo:** usuário pediu aperfeiçoar a lógica das rotinas (exceto pipeline-semanal) com aprendizados documentados (errors.md, lessons L01-L14, meta-coaching WP1-WP3).
**Ação (11 rotinas + ingest-agent):** revisao F2.2 `shuf`→função `rnd()` portável (L05, quebrava Connection Finder no macOS); vault-hot-sweep §4 `grep -oP`→`-oE`+sed + check-resolvable (L01); ingest-fiap template +Glossário/Exercícios/Por-que-importa/Tabela 8-14KB (L09); daily-scan +F1.0c cross-check `source:` (errors 06-10); daily-brief +alerta FIAP/concurso no topo (WP1); rotina-audit 6.6 wikilink score-drift `reasoning/`→`core/`; runtime-trace (WP3) em process-queue/vault-reconcile/x-thread/vault-hot-sweep; ingest-agent +F2.10 popula SRS tracker (criado `07-QUEUE/trackers/srs-sources-tracker.md`).
**Achado:** vault gitignora tudo exceto `04-SYSTEM/agents|skills` + hot.md — commit gates em process-queue/ingest-fiap/srs-sources seriam no-op (paths ignored), removidos. Só `ingest-agent.md` é git-trackable; edits de rotina são disk-only (design existente).
